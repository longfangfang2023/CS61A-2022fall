...
def boar_brawl(player_score, opponent_score):
    ...
    result=max(1,abs(ten_digit-one_digit)*3)
    return result
    # END PROBLEM 2
...
def sus_points(score):
    ...
    # BEGIN PROBLEM 4
    "*** YOUR CODE HERE ***"
    nums = num_factors(score)
    if 3 <= nums and nums<=4:
...
def is_always_roll(strategy, goal=GOAL):
    ...
    for i in range(GOAL):
        for j in range(GOAL):
            if GOAL>100 or strategy(i,j) != rolls:
                return False
    return True
    # END PROBLEM 7
...
def max_scoring_num_rolls(dice=six_sided, samples_count=1000):
    ...
    max=0
    count=0
    current_average=make_averaged(roll_dice,samples_count)
    for i in range(1,11):
        current = current_average(i,dice)
        if max < current :
            max = current
            count = i
    return count
...
def boar_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    compare_score= boar_brawl(score, opponent_score)
    if compare_score >= threshold:
        return 0
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    # BEGIN PROBLEM 11
    compare_score = sus_update(0, score, opponent_score)
    if compare_score-score >= threshold:
        return 0
    return num_rolls  # Remove this line once implemented.
    # END PROBLEM 11
...
def final_strategy(score, opponent_score):
    ...
    *** YOUR DESCRIPTION HERE ***
    ...
    # BEGIN PROBLEM 12
    compare_score = sus_update(0, score, opponent_score)
    if compare_score-score >= make_averaged(always_roll(6))(score, opponent_score):
        return 0
